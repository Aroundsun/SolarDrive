...
在安全与运维方面，内置 **JWT 鉴权**、**Redis 缓存**、**IP 滑动窗口限流**、**YAML 配置**与 **spdlog 结构化日志**；附带 **Web 控制台**与 **Prometheus 监控面板**，并支持 **Docker Compose 一键部署**（PostgreSQL + Redis + 服务本体）。
...
| **IP 限流** | Redis 滑动窗口，`limits.rate_limit_per_ip`（默认 100 次/分钟/IP） |
...
`main` 分支 push / PR 由 [GitHub Actions CI](https://github.com/Aroundsun/SolarDrive/actions/workflows/ci.yml) 自动执行 `ctest`（当前 **131** 个用例，另 1 个 Redis 集成测默认跳过）。
...
./test/cache_test
# Redis 集成测（需本机 Redis）
SOLAR_TEST_REDIS=1 ctest -R RateLimiterIntegration --output-on-failure
```
...
limits:
  rate_limit_per_ip: 100       # 每 IP 每分钟请求上限（Redis 滑动窗口；0 关闭）
