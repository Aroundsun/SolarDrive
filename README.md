# SolarDrive — 高性能 C++ 云存储系统

SolarDrive 是一个基于 **Reactor 多线程网络模型** 的自研云存储服务，使用 C++17 开发。支持文件的上传、下载、断点续传、秒传去重、用户鉴权、Redis 缓存、YAML 配置、结构化日志与 Prometheus 监控。

> 项目状态：**Phase 2 完成，Phase 3 进行中** — 核心链路稳定，已支持 Docker 一键部署。
...
| **鉴权中间件** | 白名单（健康检查、认证、静态资源、/metrics） |
...
Phase 4 🔲  功能扩展：HTTPS + Grafana + 文件分享 + 缩略图
```
